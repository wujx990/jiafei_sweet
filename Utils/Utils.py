...
def get_txt_dict():
    ...
    TXT_dict['CUB'] = ['./wjxTest/cub-train.txt',
                       './wjxTest/cub-test.txt']

    TXT_dict['DOG'] = ['./wjxTest/dog-train-20210110.txt',
                       './wjxTest/dog-test-20210110.txt']
    return TXT_dict
...
def wjx_data_dict_reader(data,phase,root):
    data_dict = get_txt_dict()
    if phase == 'train':
        if data == 'CUB':
            train_root = os.path.join(os.path.join(os.path.join(root, 'data'), 'CUB_200_2011'), 'train')  #
        elif data == 'DOG':
            train_root = os.path.join(os.path.join(os.path.join(root, 'data'), 'standford_Dog'), 'train')
        txt_path = data_dict[data][0]
    else:
        if data == 'CUB':
            test_root = os.path.join(os.path.join(os.path.join(root, 'data'), 'CUB_200_2011'), 'test')  #
        elif data == 'DOG':
            test_root = os.path.join(os.path.join(os.path.join(root, 'data'), 'standford_Dog'), 'test')
        txt_path = data_dict[data][1]
    data_dict = {}
    with open(txt_path) as f:
        txt_lines = f.readlines()
    ...
    for key, value in enumerate(txt_lines):
        info_list = value.split(' ')
        if phase == 'train':
            info_list[0] = os.path.join(train_root,info_list[0])
            #print('info_list[0] = ',info_list[0])
        else:
            info_list[0] = os.path.join(test_root, info_list[0])
        this_label_id = int(info_list[1])
        if label_id != int(this_label_id):
